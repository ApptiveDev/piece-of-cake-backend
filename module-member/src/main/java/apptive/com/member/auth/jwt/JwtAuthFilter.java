package apptive.com.member.auth.jwt;
...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    private UsernamePasswordAuthenticationToken getUserAuth(String username) {
        var userInfo = memberService.find(Long.parseLong(username));

        return new UsernamePasswordAuthenticationToken(userInfo.userId(),
                userInfo.loginPwd(),
                Collections.singleton(new SimpleGrantedAuthority(userInfo.roleName().getRole()))
        );
    }
}
